...
#ifdef _MSC_VER
#define _SCL_SECURE_NO_WARNINGS // Turn of warnings concerning iterator bound
                                // checks.
#endif
...
void splitAndTrim(const string &input, vector<string> &result,
                  const string &seperators) {
  boost::split(result, input, boost::is_any_of(seperators));
  for_each(result.begin(), result.end(),
           boost::bind(&boost::trim<string>, _1, locale()));
}
...
bool getPathFromUrl(const string &inputFileUrl, string &outputFilePath) {
#ifdef WIN32
...
/// Retrieve data from file. The file is expected to have one entry per line,
/// comment lines start with a semicolon (;). Return value is 'false' in case
/// the specified file cannot be found/opened. In case the file can be
/// found/opened, the result vector is cleared (all existing elements are
/// removed) and filled with the data provided from the file.
bool readDataFromFile(const string &file_name, vector<string> &result) {
...
bool copyFile(const boost::property_tree::ptree &fileAttributes,
  ...
  if (is_regular_file(filePath)) { // Check if regular file.
    ...
    // Copy file.
    copy_file(filePath, copyToPath, copy_options::overwrite_existing);
  } else {
    stringstream sserr;
...
// A file URI may start with "fmu://". In that case the
// FMU's location has to be prepended to the URI accordingly.
void processURI(string &uri, const string &fmuLocation) {
...
void addVectorToTree(boost::property_tree::ptree &tree,
                     const vector<string> &vector, const string &childName) {
